...
checkExpr funcoes vars (Neg expr) = do
    ...
    if (tipo == TInt || tipo == TDouble) then
        pure (tipo, Neg expr')
    else do
...
checkComando funcoes vars _ (Atrib id expr) = do
    ...
    if not (varExiste id vars) then do
        emiteErro ("Variavel '" ++ id ++ "' nao declarada.")
        pure (Atrib id (snd expr'))
    else do
        let tipo = tipoVar id vars
        let tipoexpr = fst expr'
        if tipoexpr == TVoid then do
            emiteErro ("Ocorreu algum erro na verificacao do tipo da expressao.")
            pure (Atrib id (snd expr'))
        else if tipo == tipoexpr then do
            pure (Atrib id (snd expr'))
        else if tipo == TDouble && tipoexpr == TInt then do
            emiteAviso ("Inteiro sendo convertido para double.")
            pure (Atrib id (IntDouble (snd expr')))
        else if tipo == TInt && tipoexpr == TDouble then do
            ...
            case maybeExpr of
                Just expr -> do
                    ...
                    if tipo == TVoid then do
                        ...
                        if tipo == fst expr' then do
                            pure (Ret maybeExpr)
                        else if tipo == TDouble && fst expr' == TInt then do
                            emiteAviso ("Inteiro sendo convertido para double.")
                            pure (Ret (Just (IntDouble (snd expr'))))
                        else if tipo == TInt && fst expr' == TDouble then do
                            emiteAviso ("Double sendo convertido para inteiro.")
